...
export function enrichRankings(models) {
  ...
  const scored = prepared.map(model => {
    ...
    // Fallback aggregate is deliberately transparent and uses whatever independent
    // benchmark families are actually available. Weights are re-normalized across
    // available sources. Release date never changes this score.
    ...
    const tier = arenaRank ? arenaTier(arenaRank) : aggregateTier(aggregate);
    ...
    return {
      ...
      ranking: {
        ...
        basis: arenaRank ? 'Arena Overall' : sourceCount ? `Aggregate · ${sourceCount} source${sourceCount === 1 ? '' : 's'}` : 'Awaiting independent evaluation',
        confidence,
        ...
        agent: round(agent),
        tier,
        tierBasis: arenaRank ? 'arena' : aggregate != null ? 'aggregate' : 'none',
      },
    };
  });
