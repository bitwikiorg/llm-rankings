...
export function enrichRankings(models) {
  ...
  const scored = prepared.map(model => {
    ...
    // Experimental consensus is evidence-only. Release date never changes rank.
    // At least two independent benchmark families are required.
    const consensus = sourceCount >= 2 ? weighted(overallSources) : null;
  ...
  return scored
    .sort((a, b) => (a.benchmarks?.arena?.rank || 9999) - (b.benchmarks?.arena?.rank || 9999))
