...
function neutralComposite(entries, neutral = 50) {
  ...
  const evidenceScore = available.reduce((sum, entry) => sum + Number(entry.value) * entry.weight, 0) / availableWeight;
  const coverage = availableWeight / totalWeight;
  return { score, evidenceScore, coverage };
}
...
function rankEstimateAgainstConservative(models, metric, evidenceScore) {
  if (evidenceScore == null || !Number.isFinite(Number(evidenceScore))) return null;
  const target = Number(evidenceScore);
  const scores = models
    .map(model => Number(model.scores?.[metric]))
    .filter(Number.isFinite)
    .sort((a, b) => b - a);
  if (!scores.length) return null;
  return scores.filter(score => score > target).length + 1;
}
...
export function enrichRankings(models) {
  ...
  const estimateMetrics = ['overall', 'reasoning', 'coding', 'agent'];
  ...
  return scored
    .map(({ _blendedCost, ...model }) => {
      ...
      const rankEstimates = Object.fromEntries(estimateMetrics.map(metric => {
        const coverage = Number(model.coverage?.[metric] ?? 0);
        const conservativeRank = metricRanks[metric];
        const evidenceScore = model.evidenceScores?.[metric];
        if (coverage < 55 || coverage >= 100 || conservativeRank == null || evidenceScore == null) return [metric, null];
        const estimatedRank = rankEstimateAgainstConservative(scored, metric, evidenceScore);
        if (estimatedRank == null) return [metric, null];
        return [metric, {
          estimatedRank,
          conservativeRank,
          best: Math.min(estimatedRank, conservativeRank),
          worst: Math.max(estimatedRank, conservativeRank),
          evidenceScore,
          coverage,
          method: 'available-evidence extrapolation',
        }];
      }));
